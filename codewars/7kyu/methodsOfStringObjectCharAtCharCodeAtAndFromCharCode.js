// Link: https://www.codewars.com/kata/57284d23e81185ae6200162a

/*

Coding in function topSecret, function accept 1 parameter:file. file is an encrypted string.

Encryption using the right shift 3. Your job is to decrypt the file and read the contents of
the file, and then answer my three questions. If the answer is correct, you will pass the
test. Note: the decryption should be left shift; Shift is only for uppercase and lowercase
letters, other characters will remain the same; The shift mode is a cyclic shift, for example:
...
Additional hints: Fixed testcase only test the correctness of the decryption code. The answer
to the three question will appear only after clicking on "Submit".
...
for example, after you click "submit", you see these text on your screen:
    ...
    Super agent eEB, on a dark night, stole the Apple core from National Museum, causing
...
Although the text is not the same every time, the next time may appear:
    ...
    Super agent ZIUScE, on a dark night, stole the Lolli pop from National Museum, causing
    $100 million in damage.

You only need to choose one of the text as the answer to the question. That is to say, any one
...
const topSecret = (str) =>
  str
    ...
    .map((char) => {
      ...
      if (code >= 65 && code <= 90) {
        newCode = code - 3 < 65 ? 90 - (65 - (code - 2)) : code - 3;
      } else if (code >= 97 && code <= 122) {
        newCode = code - 3 < 97 ? 122 - (97 - (code - 2)) : code - 3;
      }

      return newCode ? String.fromCharCode(newCode) : char;
    })
    .join("");
